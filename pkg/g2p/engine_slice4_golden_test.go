...
// Slice 4 golden support for the OOV fallback corpus (ADR-0009).
//
...
// Slice-5 golden.Record / TestGolden machinery for two reasons:
//
//  1. The frozen Phase-1 golden files (words/oov/atypical/nonword/causal) do
//     NOT carry a fallback_rules_version field, and they must not be modified
//     or regenerated by this slice. Adding the field to the shared
...
// Phase-1 schema and adds fallback_rules_version immediately after oov_policy
// (ADR-0009 metadata requirement).
...
// fallbackGoldenCases is the authoritative ordered list of Slice-4 golden
...
func TestGoldenFallbackExactMatch(t *testing.T) {
	...
	for i, c := range cases {
		...
		// Explicit field-level assertions required by the slice spec.
