...
// (ADR-0010, Decision §5). The mini-dict identifier remains EmbeddedID.
const FullID = "cmudict-full-v0.7b"

// FullExpectedSHA256 is the frozen hex-encoded SHA-256 digest the loader
// compares against (ADR-0010, Checksum Policy). The value is part of this
// ADR's design freeze; changing it requires bumping FullID (e.g. to
...
// fallback to the mini-dict (ADR-0010, Fallback Policy) until a future commit
// (Phase 3 / Slice ≥ 3) pins the true digest of the upstream artifact.
// Contributors must not edit this constant locally to "match their copy" —
// see ADR-0010, Categorical Prohibitions ("no checksum bypass").
...
var (
	...
	// the [1 MiB, 16 MiB] sanity band (ADR-0010, Loading Pipeline §3). This
	// is a short-circuit guard, NOT a substitute for the SHA-256 check.
	...
	// does not match FullExpectedSHA256 (ADR-0010, Checksum Policy). There is
	// no bypass flag.
	...
	// parser receives a non-CMUdict payload (ADR-0010, Loading Pipeline §5).
